...
stop({SessionKey, SessionServerPid}) ->
    ...
    stop(ets:next(session_list, {SessionKey, SessionServerPid})).
...
get_session_pid(SessionKey) ->
    ...
    case SessionList of
        [] ->
            ...
            io:format("Creating new session [SessionKey:~s] [SessionServerPid:~w] ~n", [SessionKey, SessionServerPid]),
            SessionServerPid;
        [{SessionKey, SessionServerPid} ] ->
            io:format("Using existing session [SessionKey:~s] [SessionServerPid:~w] ~n", [SessionKey, SessionServerPid]),
            SessionServerPid
    end.
